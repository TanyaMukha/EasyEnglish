# EasyPeasy.ContentTools

A personal console tool for authoring course content — not part of the running app, and not
referenced by any other project except `EasyPeasy.Core` (for the model types it edits). Run with
...
EasyPeasy app through its own "Import Course ZIP" feature
(`EasyPeasy.App/Components/Pages/ImportCourseZip.razor`, backed by
`EasyPeasy.App/Services/CourseZipBackupService.cs`). `CourseZipEditor.JsonOpts` is deliberately
