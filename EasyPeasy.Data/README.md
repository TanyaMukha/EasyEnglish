...
implementations behind `EasyPeasy.Core`'s `I*Repository` interfaces. Everything here is EF
Core/SQLite-specific — `EasyPeasy.Business` and `EasyPeasy.App` depend on the `I*Repository`
interfaces from `EasyPeasy.Core`, never on this project's concrete types directly (only the
DI registration extension, `AddEasyPeasyRepositories()`, is called from app startup).

## Project layout

| File / folder | Purpose |
|---|---|
| `EasyPeasyDbContext.cs` | The `DbContext`. No `OnModelCreating` override — schema comes entirely from data annotations on `EasyPeasy.Core.Entities.*`. Also auto-stamps `CreatedAt`/`UpdatedAt` on save. |
| `DesignTimeDbContextFactory.cs` | `IDesignTimeDbContextFactory<EasyPeasyDbContext>` — used only by `dotnet ef` CLI commands, not at app runtime. |
| `Extensions/ServiceCollectionExtensions.cs` | `AddEasyPeasyRepositories()` — registers all 8 repositories. Does *not* register `IDbContextFactory<EasyPeasyDbContext>` or `IUserContext`; those come from `EasyPeasy.App`'s startup code. |
...
The schema is entity-first: `EasyPeasy.Core.Entities.*`'s `[Table]`/`[Column]`/`[MaxLength]`/
...
Every repository follows the same shape: constructor takes `IDbContextFactory<EasyPeasyDbContext>`
...
3. **`IrregularFormRepository`/`IIrregularFormRepository` has no `GetNavigationIdsAsync`**, unlike
   ...
   swipe UI for irregular forms in `EasyPeasy.App`), not an oversight. Left as-is.
...
`EasyPeasy.Data.Tests` (36 tests) uses a real SQLite in-memory `EasyPeasyDbContext` (one open
...
framework used for `EasyPeasy.Core`:
...
- **`EasyPeasyDbContext.UpdateAuditInfo`** — `CreatedAt` set on insert, `UpdatedAt` set on update,
