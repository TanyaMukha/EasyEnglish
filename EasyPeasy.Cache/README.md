...
An in-memory "working set" cache layer for `EasyPeasy.App` — not a general-purpose caching
...
`EasyPeasy.Core.Interfaces.Storage.IStorageService`, so the app can restore "what was selected"
...
| File | Purpose |
|---|---|
| `Extensions/ServiceCollectionExtensions.cs` | `AddEasyPeasyCacheServices()` — registers `ICurrentUnitCacheService`/`IWordCacheService` as singletons. |
...
1. ~~**`AddSingleton<ICurrentUnitCacheService>`/`AddSingleton<IWordCacheService>` depended on
   `IUnitService`/`IWordService` directly**, which `EasyPeasy.Business` registers `AddScoped` — a
...
`EasyPeasy.Cache.Tests` (19 tests). No SQLite needed — this library has no EF Core/database
