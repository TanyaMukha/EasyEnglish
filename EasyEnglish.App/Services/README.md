# EasyEnglish.App/Services

App-local services for `EasyEnglish.App` (the MAUI Blazor Hybrid host) — everything under
`Services/`, `Services/Speech/`, and `Services/SpeechRecognition/`. Unlike `EasyEnglish.Business`
...
reason to live in a reusable library. This document does not cover `EasyEnglish.App`'s Razor
components — only the `Services` folder.

Two files live in the bare `namespace EasyEnglish.Services;` (no `.App` segment) rather than
`EasyEnglish.App.Services`: `FileService.cs` and `CourseZipBackupService.cs`. This is a pre-existing
naming quirk, not a typo — don't confuse it with `EasyEnglish.Business.Services`, the shared
...
| `CourseZipBackupService.cs` | Exports/imports a whole course as a ZIP archive (`course.json` manifest + per-unit JSON + audio files). `namespace EasyEnglish.Services;`. |
...
| `FileService.cs` | Cross-platform save/pick-file operations (`FilePicker`, `Share`, platform-specific save paths for Android/iOS/macOS). `namespace EasyEnglish.Services;`. |
| `HomeStatsService.cs` | Home-page stats (today's review count, library totals) computed live from `EasyEnglish.Business` services — no separate progress bookkeeping. |
...
| `LocalStorageService.cs` | The real `IStorageService` implementation — MAUI `Preferences` + JSON. Backs `EasyEnglish.Cache`'s cache services and everything below that takes `IStorageService`. |
...
*(`ISpeechEngine`, `IVoiceProvider`, `ISpeechService` are declared in `EasyEnglish.App/Interfaces/`, not this folder, but are documented alongside their implementations since they're this subsystem's contracts.)*
...
`IPronunciationCheckService` (in `EasyEnglish.App/Interfaces/`) is likewise documented alongside its implementation for the same reason.
...
`EasyEnglish.App.Tests` (103 tests) covers the pure-logic pieces with no MAUI dependency —
...
**Why a separate project instead of referencing `EasyEnglish.App.csproj` directly**: `EasyEnglish.App`
...
standalone. Instead, `EasyEnglish.App.Tests` targets plain `net9.0` (no MAUI) and pulls in the
...
`ProjectReference` to `EasyEnglish.Core.csproj` for the model types (`WordModel` etc.) that
