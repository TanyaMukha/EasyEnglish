...
every `EasyEnglish.*`/`MukhaLab.*` project. If you're about to be surprised by something, check here
...
- **`MukhaLab.*` is reserved for genuinely reusable, non-EasyEnglish-specific code** — no course/word/
  unit/English-grammar domain knowledge baked in. Something that's "not MAUI-coupled" but still
  encodes EasyEnglish's domain (e.g. `HomeStatsService`, `WordRatingCalculator`) does **not**
  qualify, even though it could technically compile in isolation.
- **Namespace trap**: `EasyEnglish.Services` (bare, no `.App` or `.Business` segment) is used by
  a handful of `EasyEnglish.App`-local classes (`FileService`, `CourseZipBackupService`) — it is
  **not** the same thing as `EasyEnglish.Business.Services` (the actual business/domain service
  layer, renamed from the historical `EasyEnglish.Services.Services`). The two are easy to conflate
  because they share the `EasyEnglish.Services` prefix; check the `using`/project, not just the
  short name, when you see `XyzService`.
- **`AddEasyEnglishDataServices()` lives in `EasyEnglish.Business`, not `EasyEnglish.Data`.** It
  registers the business/domain services (`WordService`, `UnitService`, etc.) — the name is a
  leftover and doesn't match its own project. `EasyEnglish.Data`'s equivalent method is
  `AddEasyEnglishRepositories()`.

## Language: code vs. UI text

- **All code comments, XML doc comments, and identifiers are English** — including on `internal`
  types (e.g. `TextChunkParser` in `EasyEnglish.App.Services.Speech`). No exceptions for
  visibility level.
...
- **A singleton that needs a scoped dependency must take `IServiceScopeFactory` and create a
  ...
  per logical operation). See `EasyEnglish.Cache`'s `WordCacheService`/`CurrentUnitCacheService` for
  the pattern.
- **`EasyEnglish.App/MauiProgram.cs` is the composition root** — the only place that references both
  `EasyEnglish.Data` and `EasyEnglish.Business` and wires the former's concrete repositories to the
  latter's interfaces. `EasyEnglish.Business` itself only depends on interfaces declared in
  `EasyEnglish.Core` (see [key-decisions.md](../Decisions/key-decisions.md)) — don't add a
  `EasyEnglish.Data` project reference to `EasyEnglish.Business` to "make it easier" to call a
...
- **Don't `ProjectReference` a MAUI head project (`UseMaui=true`, `OutputType=Exe`) from a test
  ...
  `<Compile Include>`-linking workaround `EasyEnglish.App.Tests` uses instead.
- **If a `dotnet build`/`dotnet test` involving `EasyEnglish.App`'s Windows target fails with a
  ...
  `EasyEnglish.App/obj` and `EasyEnglish.App/bin` and rebuild.
