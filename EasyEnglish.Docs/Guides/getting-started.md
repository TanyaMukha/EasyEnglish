...
Developer onboarding for the EasyEnglish solution. If you just want an overview of what each
...
git clone <repo-url>
cd EasyEnglish
dotnet restore EasyEnglish.sln
```

## Build

The solution has one MAUI multi-targeted project (`EasyEnglish.App`, targets
...
```
dotnet build EasyEnglish.sln
```

To build just the Windows app (fastest local loop):

```
dotnet build EasyEnglish.App/EasyEnglish.App.csproj -f net9.0-windows10.0.19041.0
```
...
times during development, unrelated to any specific change. Fix: delete `EasyEnglish.App/obj` and
`EasyEnglish.App/bin`, then rebuild.

## Run

From Visual Studio: set `EasyEnglish.App` as the startup project, pick the "Windows Machine" debug
target, and press F5.

From the CLI (Windows):

```
dotnet build EasyEnglish.App/EasyEnglish.App.csproj -t:Run -f net9.0-windows10.0.19041.0
```

No manual database setup is needed — `appsettings.json` has `Database:AutoMigrate` and
`Database:SeedInitialData` both `true`, so the SQLite file (`{AppDataPath}/EasyEnglish.db`) is
...
```
dotnet test EasyEnglish.Core.Tests/EasyEnglish.Core.Tests.csproj
```
...
```bash
for p in EasyEnglish.Core.Tests EasyEnglish.Data.Tests EasyEnglish.Business.Tests \
         EasyEnglish.Cache.Tests EasyEnglish.App.Tests \
...
```powershell
$projects = "EasyEnglish.Core.Tests","EasyEnglish.Data.Tests","EasyEnglish.Business.Tests",
            "EasyEnglish.Cache.Tests","EasyEnglish.App.Tests",
            "MukhaLab.SelectQueryParameters.Tests","MukhaLab.Database.Tests",
...
`EasyEnglish.Data.Tests`, `MukhaLab.Database.Tests`, and `EasyEnglish.Business.Tests` (for its
