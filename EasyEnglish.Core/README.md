...
The domain layer of the EasyEnglish app: EF Core entities, UI/serialization-facing DTO models, the
...
and contracts. `EasyEnglish.Data` implements the repository interfaces against SQLite;
`EasyEnglish.Business` implements the service interfaces; `EasyEnglish.App` (MAUI Blazor Hybrid)
...
[`EasyEnglish.Docs/Diagrams/entities.mdpuml`](../EasyEnglish.Docs/Diagrams/entities.mdpuml) and
[`database.mdpuml`](../EasyEnglish.Docs/Diagrams/database.mdpuml).
...
| Folder | Contents |
|---|---|
| `Entities/` | EF Core-mapped persistence classes (`[Table]`, `[Column]`, `[MaxLength]`, ...). Never leave `EasyEnglish.Data`. |
...
attributes and EF navigation properties, and are only ever touched inside `EasyEnglish.Data`.
...
5. **`Presets/*Includes` are unchecked strings.** Renaming a navigation property won't cause a
   ...
   `DbContext`, which belongs in `EasyEnglish.Data`'s own test suite (this library has no `DbContext`
   of its own), not here.
...
doesn't already guarantee). `EasyEnglish.Core.Tests` (50 tests) covers the pockets of real logic that do:
