...
public class WordMappingOptions
{
    ...
    /// <summary>When <c>true</c>, drops the word's <see cref="EasyEnglish.Core.Models.WordModel.Examples"/>.</summary>
...
/// <summary>
/// Options controlling a <c>UnitModel → UnitModel</c> self-map (see <see cref="EasyEnglish.Core.Mapping.UnitMappingAction"/>),
/// passed via <c>mapper.Map(unit, opts => opts.Items[UnitMappingOptions.Key] = new UnitMappingOptions { ... })</c>.
/// Note: this only affects <see cref="EasyEnglish.Core.Models.UnitModel.Words"/> and their nested
/// <see cref="EasyEnglish.Core.Models.ExampleModel"/> — <c>IrregularForm</c>/<c>StudyCard</c>/<c>TestCard</c>
...
public class UnitMappingOptions
{
    ...
    /// <summary>When <c>true</c>, assigns a new <see cref="EasyEnglish.Core.Interfaces.Fields.IGuidInfo.RecordGuid"/>.</summary>
