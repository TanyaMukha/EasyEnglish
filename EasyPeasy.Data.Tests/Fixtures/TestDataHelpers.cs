...
public static class TestDataHelpers
{
    ...
    /// <summary>
    /// Overwrites <c>created_at</c> directly via raw SQL, bypassing <see cref="EasyPeasyDbContext"/>'s
    ...
    /// </summary>
    public static Task SetWordCreatedAtAsync(EasyPeasyDbContext ctx, int wordId, DateTime createdAt) =>
        ctx.Database.ExecuteSqlInterpolatedAsync($"UPDATE words SET created_at = {createdAt} WHERE id = {wordId}");

    public static Task SetUnitCreatedAtAsync(EasyPeasyDbContext ctx, int unitId, DateTime createdAt) =>
